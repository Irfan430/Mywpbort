...
<!-- Additional features added in this upgraded version -->
- **Utility commands**: Extra built‑in commands for convenience – echo back your message (`/echo`), reverse text (`/reverse`), evaluate arithmetic (`/calc`) and show the current date/time (`/time`). These commands live under the tools category and require no additional setup.
